...
- **Loop cap.** Cap each automated loop (review→develop, CI→develop, bot-review) at **3 iterations**. On the 3rd failure, stop and surface to the user via AskUserQuestion instead of looping again.
...
- **Surface decisions, don't bury them.** On a blocking gap or hard failure, bring it to the user with AskUserQuestion before burning another develop loop — unless the fix is unambiguous.

...
6. **Final gate (7):** run verify Phase 2 with the user (skip if non-UI), then notify + report the green PR URL and that it awaits their merge. **Stop — do not merge.** Worktree cleanup is deferred until the PR is merged: leave the run open (`Step: awaiting-merge`), and **on a later `/deliver` invocation (or when the user says it's merged) first check the state file's `PrNumber` — if `gh pr view` shows merged, reap the worktree + branch, close the ticket, and mark the run complete** before doing anything else. **Ticket closure is part of the flow:** transition the issue to the repo's terminal status (from `## Delivery configuration`; default Done) and add a brief closing comment — what shipped (PR links), verification outcome, follow-ups. Keep it concise.

---
name: deliver
description: Take a Jira ticket, an idea, or a plan file all the way to a review-ready PR. Phase 0 writes a plan and pauses for approval; subagents then create the issue, develop in a worktree, review the diff against the plan alongside the native /security-review skill, open the PR, and watch CI until green. Never merges.
...
- **Small diff** (single file, ~≤50 lines, no new module): skip Step 2; Step 4 runs the correctness reviewer alone.
- **Multi-file / security-sensitive / wire-format / large:** Step 2, then Step 4 runs the correctness reviewer + `/security-review` in parallel.
...
- **Spawn contract:** give each step subagent its step file's absolute path (they sit **beside** `SKILL.md` — there is no `steps/` subdir), `PLAN_PATH`, `REPO_ROOT`, `WORKTREE`+`BRANCH` once known, `Ticket`+`Slug`, and your addressable name — pass what exists at that point: the **plan subagent mints the slug and plan filename**; you learn `PLAN_PATH` + `Slug` from its return. Use the default general-purpose agent — steps 0/3/4 need the Skill tool. **Returns arrive only via SendMessage**; a parallel subagent's plain-text output is not relayed (idle ping = nudge it once to SendMessage; still silent → respawn that step once, then surface to the user). Messages can arrive out of order, duplicated, or **stale** (a nudge can elicit a resend of an old return that predates your latest instruction) — before relaying any claimed write as done, spot-check the artifact itself (grep the file, fetch the ticket/PR), and check git state before acting on sequencing.
- **`/code-review` and `/verify` are user-invocable-only.** The platform refuses them to every subagent — no setting, prompt, or spawn shape changes that, so don't "fix" this back and don't route around it with a nested `claude -p`. They are the human's passes; `/code-review` already forks its own reviewer when you run it. The delegated equivalents are Step 4's correctness reviewer and Step 3's repo `verifier-*` skill, both ordinary model-invocable skills.
- **Develop commits; you push.** Develop's reported Phase-1 verify *is* the build gate — you run no builds/tests yourself; your pre-push check is provenance only (every change came from a subagent). Never tell a subagent "don't push" and later ask the same one to push — that boundary can't be lifted. **A SHA with no reported verify has no gate:** if Develop dies mid-task (API/spend-limit failure) or the base moved under it, re-establish the gate before review — respawn, or run it yourself and say so.
- **Re-derive the base tip before every spawn** (`git fetch` + `rev-parse origin/<base>`) and pass it as a fact to be re-checked, not trusted. The base moves mid-flow — a merge during Phase 0 silently invalidates a plan grounded on the old tip, and briefing a stale SHA sends subagents to re-derive or contradict you.
- **Fixes get a NEW commit — never amend.** Every fix turn commits on top, so each review-driven change is visible as its own diff. Never instruct a subagent to amend, and if one offers to ("it reads cleaner as one commit"), decline. Amending a pushed branch forces a `--force-with-lease` that invalidates the CI run and review-bot pass already in flight on the old head, costing a review round as well as the history.
...
| 3 | **Develop** — reuse-or-create worktree (`<KEY>-<slug>`), implement, verify **Phase 1** via the repo's `verifier-*` skill (repo CI gate is a sanity check, not the pass), commit | `03-develop.md` | worktree + branch + verify evidence + notes |
| 4 | **Review** — up to 2 parallel agents, one role each: **correctness** (diff vs. plan, always) + **security** (`/security-review`, full reviews); a fresh context judges the result, never the author | `04-review.md` | per-role findings + severity |

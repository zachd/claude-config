# Deliver Phase 0 — Spec subagent

**You are a spec/planning subagent.** You were given the raw input (a Jira ticket URL/KEY, a freeform idea, and/or a reference file) and `REPO_ROOT`. Produce a grounded draft spec **file** and return a compact summary plus the open decisions the orchestrator must resolve with the user. **Do not implement anything. Do not run the approval gate yourself** — the orchestrator owns the user conversation.
...
3. **Ground in the codebase**: launch **1–3 Explore subagents in parallel** to find existing patterns/utilities to reuse, the files the change will touch, the established mechanism for this kind of work (so the spec reuses it rather than reinventing), and how similar features are verified here. Keep their file dumps out of your return — synthesize.
4. **Write the draft spec file** to `REPO_ROOT/.claude/specs/<slug>.md` (create the dir). Use the format below.
...
## Spec file format (header keys are machine-read — keep them exact)
...
```
STATUS: pass | blocked
SPEC_PATH: <absolute path to the draft spec file>
TITLE: <spec title>
TICKET: <KEY or none>
...
If you cannot ground the request at all (e.g. ticket unreadable, request incoherent), return `STATUS: blocked` with why. Note: the spec dir `.claude/specs/` should be gitignored unless the repo intends to commit specs — flag this to the orchestrator if `.gitignore` doesn't cover it.

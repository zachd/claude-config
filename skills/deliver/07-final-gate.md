...
1. **Decide if Phase 2 is needed.** Read the spec's `## Files to touch` / `TOUCHES_UI` flag.
   ...
   - **UI touched** → run **verify Phase 2**: ask the user to confirm the UI renders and behaves as expected, summarizing the Phase-1 findings and what to look at. Offer clear options (works / has an issue / skip — logs sufficient). Use the repo's `/verify` skill's Phase-2 wording if it has one.
...
- `gh pr view <PrNumber> --json state,mergedAt` — if **merged**:
  ...
  2. Mark the run complete (state file `Step: done`; master task `completed` if present).
  3. Tell the user the worktree/branch were reaped.
